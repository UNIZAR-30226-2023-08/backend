...
class Partida2Torneo:
    ...
    async def add_player(self, websocket: WebSocket, client_id: str):
        ...
        if self.jugadores == 2:
            ganador = await self.iniciar_partida()
    ...
    async def await_message(self, id):
        try:
            if id == "0":
                mensaje_jugador_0 = await asyncio.wait_for(self.sockets["socket0"].receive_text(), timeout=120)
                return mensaje_jugador_0
            else:
                mensaje_jugador_1 = await asyncio.wait_for(self.sockets["socket1"].receive_text(), timeout=120)
    ...
    async def await_message_siete(self, id):
        try:
            if id == "0":
                mensaje_jugador_0 = await asyncio.wait_for(self.sockets["socket0"].receive_text(), timeout=2)
                return mensaje_jugador_0
            else:
                mensaje_jugador_1 = await asyncio.wait_for(self.sockets["socket1"].receive_text(), timeout=2)
                return mensaje_jugador_1
        except asyncio.TimeoutError:
            return None
    ...
    async def actualizarEstadisticas2Jugadores(jugadorGanador, jugadorPerdedor):
